...
class MixSGAN:
    def __init__(self, args):
        ...
        enc_layer_conv1 = LL.Conv2DLayer(self.enc_layers[-1], 64, (5, 5), pad=0, stride=1, W=Normal(0.01),
        ...
        enc_layer_conv2 = LL.Conv2DLayer(self.enc_layers[-1], 128, (5, 5), pad=0, stride=1, W=Normal(0.01),
                                             nonlinearity=nn.relu)
    ...
    def get_generator(self, meanx, z0, y_1hot):
        ...
        gen_x_layer_y = LL.InputLayer(shape=(self.args.batch_size, 10),
        ...
        gen_x_layer_pool2 = LL.ReshapeLayer(nn.batch_norm(LL.DenseLayer(gen_x_layer_y_z, num_units=256 * 5 * 5)),
                                            (self.args.batch_size, 256, 5, 5))
        gen_x_layer_dconv2_1 = nn.batch_norm(
            nn.Deconv2DLayer(gen_x_layer_pool2, (self.args.batch_size, 256, 10, 10), (5, 5), stride=(2, 2),
                             padding='half',
                             W=Normal(0.02), nonlinearity=nn.relu))
        gen_x_layer_dconv2_2 = nn.batch_norm(
            nn.Deconv2DLayer(gen_x_layer_dconv2_1, (self.args.batch_size, 128, 14, 14), (5, 5), stride=(1, 1),
                             padding='valid',
                             W=Normal(0.02), nonlinearity=nn.relu))

        gen_x_layer_dconv1_1 = nn.batch_norm(
            nn.Deconv2DLayer(gen_x_layer_dconv2_2, (self.args.batch_size, 128, 28, 28), (5, 5), stride=(2, 2),
                             padding='half',
                             W=Normal(0.02), nonlinearity=nn.relu))
        gen_x_layer_x = nn.Deconv2DLayer(gen_x_layer_dconv1_1, (self.args.batch_size, 3, 32, 32), (5, 5), stride=(1, 1),
                                         padding='valid',
    ...
    def get_discriminator(self):
        ...
        disc_x_layers.append(
            LL.Conv2DLayer(disc_x_layers[-1], 96, (3, 3), pad=1, W=Normal(0.01), nonlinearity=nn.lrelu))
        disc_x_layers.append(nn.batch_norm(
            LL.Conv2DLayer(disc_x_layers[-1], 96, (3, 3), pad=1, stride=2, W=Normal(0.01), nonlinearity=nn.lrelu)))
        disc_x_layers.append(LL.DropoutLayer(disc_x_layers[-1], p=0.5))
        disc_x_layers.append(nn.batch_norm(
            LL.Conv2DLayer(disc_x_layers[-1], 192, (3, 3), pad=1, W=Normal(0.01), nonlinearity=nn.lrelu)))
        disc_x_layers.append(nn.batch_norm(
            LL.Conv2DLayer(disc_x_layers[-1], 192, (3, 3), pad=1, stride=2, W=Normal(0.01), nonlinearity=nn.lrelu)))
        disc_x_layers.append(LL.DropoutLayer(disc_x_layers[-1], p=0.5))
        disc_x_layers.append(nn.batch_norm(
            LL.Conv2DLayer(disc_x_layers[-1], 192, (3, 3), pad=0, W=Normal(0.01), nonlinearity=nn.lrelu)))
    ...
    def train(self):
        ...
        for i in range(self.args.ng):
            self.y_recon_list.append(LL.get_output(self.enc_layer_fc4, self.Gen_x_list[i],
                                                   deterministic=True))  # reconstructed pool3 activations
        ...
        for i in range(self.args.ng):
            ...
            for j in range(self.args.nd):
                ...
                output_before_softmax_gen0, recon_z0 = LL.get_output([self.D_layer_adv[j], self.D_layer_z_recon[j]],
                                                                     self.Gen_x_list[i],
                                                                     deterministic=False)  # discriminator's predicted probability that gen_x is real
                ...
                loss_disc0_class += T.dot(self.D_weights[0, j], -T.mean(l_lab0) + T.mean(T.mean(nn.log_sum_exp(
                    output_before_softmax_real0))))  # loss for not correctly classifying the category of real images
        ...
        for i in range(self.args.nd):
            ...
            Disc_bn_updates.extend(
                [u for l in LL.get_all_layers(self.D_layers[i][-1]) for u in getattr(l, 'bn_updates', [])])
        ...
        Disc_param_updates = nn.adam_conditional_updates(Disc_params, self.loss_disc, mincost=self.threshold,
                                                         lr=self.disc_lr,
                                                         mom1=0.5)  # if loss_disc_x < mincost, don't update the discriminator
        Disc_param_avg = [th.shared(np.cast[th.config.floatX](0. * p.get_value())) for p in
                          Disc_params]  # initialized with 0
        Disc_avg_updates = [(a, a + 0.0001 * (p - a)) for p, a in
                            zip(Disc_params, Disc_param_avg)]  # online update of historical parameters
        ...
        for i in range(self.args.ng):
            ...
            Gen_bn_updates.extend(
                [u for l in LL.get_all_layers(self.G_layers[i][-1]) for u in getattr(l, 'bn_updates', [])])
        ...
        train_batch_disc = th.function(
            inputs=[self.dummy_input, self.meanx, self.x, self.y, self.y_1hot, self.mincost, self.disc_lr],
            outputs=[self.loss_disc0_class, self.loss_disc0_adv],
            updates=Disc_param_updates + Disc_bn_updates + Disc_avg_updates)
        ...
        train_batch_gen = th.function(inputs=[self.dummy_input, self.meanx, self.y, self.y_1hot, self.gen_lr],
                                      outputs=[self.loss_gen0_adv, self.loss_gen0_cond, self.loss_gen0_ent],
                                      updates=Gen_param_updates + Gen_bn_updates)
        ...
        reconfun = th.function(inputs=[self.meanx, self.y_1hot],
                               outputs=self.Gen_x)  # reconstruction function: use the bottom generator
        ...
        for epoch in range(load_e + 1, self.args.num_epoch):
            ...
            for t in range(nr_batches_train):
                ...
                l_disc0_class, l_disc0_adv = train_batch_disc(0.0, meanimg, batchx, batchy, batchy_1hot,
                                                              self.args.mincost, self.args.disc_lr)
                ...
                for i in range(n_iter):
                    ...
                    l_gen0_adv, l_gen0_cond, l_gen0_ent = train_batch_gen(0.0, meanimg, batchy, batchy_1hot,
                                                                          self.args.gen_lr)
                ...
                print("D_weights=[%.6f, %.6f, %.6f, %.6f, %.6f] loss = %0.6f" % (
                d_mix_weights[0, 0], d_mix_weights[0, 1], d_mix_weights[0, 2], d_mix_weights[0, 3], d_mix_weights[0, 4],
                d_entloss))
                print("G_weights=[%.6f, %.6f, %.6f, %.6f, %.6f] loss = %0.6f" % (
                g_mix_weights[0, 0], g_mix_weights[0, 1], g_mix_weights[0, 2], g_mix_weights[0, 3], g_mix_weights[0, 4],
                g_entloss))
                # print("G_weights=[%.6f]" % (g_mix_weights[0,0]))
                print(
                "loss_disc0_adv = %.4f, loss_gen0_adv = %.4f,  loss_gen0_cond = %.4f, loss_gen0_ent = %.4f, loss_disc0_class = %.4f" % (
            ...
            if epoch % self.args.save_interval == 0:
                ...
                np.savez(self.args.out_dir + '/Dweights_params_epoch{}.npz'.format(epoch),
                         *LL.get_all_param_values(self.D_weights_layer))
                np.savez(self.args.out_dir + '/Gweights_params_epoch{}.npz'.format(epoch),
                         *LL.get_all_param_values(self.G_weights_layer))
                for i in range(self.args.ng):
                    np.savez(self.args.out_dir + ("/disc%d_params_epoch%d.npz" % (i, epoch)),
                             *LL.get_all_param_values(self.D_layers[i]))
                    np.savez(self.args.out_dir + ("/gen%d_params_epoch%d.npz" % (i, epoch)),
                             *LL.get_all_param_values(self.G_layers[i]))
            ...
            for i in range(self.args.ng):
                reconx_i = np.transpose(np.reshape(reconx[i * self.args.batch_size:(i + 1) * self.args.batch_size],
                                                   (self.args.batch_size, 3, 32, 32)), (0, 2, 3, 1))
